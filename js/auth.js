// Local "database" of accounts, just localStorage for now.
function getUsers() {
...
// Text for the login/signup pages in both languages.
...
// Another tab changed the language — repaint only, never call setAppLang
// here or we'd loop.
window.addEventListener(APP_LANG_EVENT, function (e) {
  applyAuthLang(e.detail);
});
...
function shakeCard() {
  ...
  void card.offsetWidth; // restart the animation if it's already playing
